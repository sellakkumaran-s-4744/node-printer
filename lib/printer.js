...
// Load native module with ABI-aware fallback
function loadNativeModule() {
    var abi = process.versions.modules;
    var platform = process.platform;
    var arch = process.arch;
    
    // Try ABI-specific binary: node_printer_<platform>_<arch>_abi<version>.node
    var abi_specific_path = path.resolve(__dirname, './node_printer_' + platform + '_' + arch + '_abi' + abi + '.node');
    if (fs.existsSync(abi_specific_path)) {
        return require(abi_specific_path);
    }
    
    // Fall back to building from source if binary not found
    console.warn('Prebuilt binary not found at ' + abi_specific_path + ', building from source...');
    throw new Error('Native module not found. Please ensure the module is built via: npm run build');
}

try {
    printer_helper = loadNativeModule();
} catch (err) {
    console.error('Failed to load printer native module:', err.message);
    throw err;
}
